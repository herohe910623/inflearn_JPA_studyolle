...
public class EventController {
    ...
    public String updateEventForm(@CurrentAccount Account account, @PathVariable String path, @PathVariable("id") Event event,
                                  Model model) {
        Study study = studyService.getStudyToUpdate(account, path);
//        Event event = eventRepository.findById(id).orElseThrow(); @PathVariable Long id인 파라미터를 Event로 변경해서 엔티티 컨버터 사용하였음.
        model.addAttribute(study);
        model.addAttribute(account);
        model.addAttribute(event);
...
//        Enrollment enrollment = enrollmentRepository.findById(enrollmentId).orElseThrow();
    ...
    public String checkInEnrollment(@CurrentAccount Account account, @PathVariable String path, @PathVariable("eventId") Event event,
                                    @PathVariable("enrollmentId") Enrollment enrollment) {
        Study study = studyService.getStudyToUpdate(account, path);
    ...
    public String cancelCheckInEnrollment(@CurrentAccount Account account, @PathVariable String path, @PathVariable("eventId") Event event,
                                          @PathVariable("enrollmentId") Enrollment enrollment) {
        Study study = studyService.getStudyToUpdate(account, path);
